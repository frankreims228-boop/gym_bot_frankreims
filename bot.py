...
async def replace_menu(callback: CallbackQuery):
    ...
    variants = {
        ...
        "Молотки": ["Скамья Скотта", "Сгибания на блоке", "Подъём гантелей на бицепс"],

        "Жим лёжа": ["Жим в хаммере", "Жим гантелей лёжа"],
        "Жим гантелей лёжа": ["Жим лёжа", "Жим в хаммере"],
        "Жим в хаммере": ["Жим лёжа", "Жим гантелей лёжа"],
    ...
    if not options:
        await callback.message.answer("Для этого упражнения пока нет быстрых замен.")
        await callback.answer()
        return
